...
3. ユーザーが答えやすく、投稿の魅力を高める質問に絞り、最大5問で情報を引き出す

【書き方ルール】
- 事実の捏造禁止。不明は「不明」と書く
- 便益は「対象ユーザー + 具体的な価値 + 使用場面」の形式で記述
- NSFW/暴力/差別/違法行為の助長にあたる語は使わない
- 日本語、簡潔・具体
- headlineIdeasのタイトル案は必ず19文字以内で作成（空白・記号含む）
...
JSONのみで返してください：
{
  "advice": {
    ...
    "headlineIdeas": ["19文字以内の案1", "19文字以内の案2", "19文字以内の案3"],
    "goodPoints": ["対象ユーザーが具体的な価値を使用場面で得られる便益1", "対象ユーザーが具体的な価値を使用場面で得られる便益2", "対象ユーザーが具体的な価値を使用場面で得られる便益3"]
  },
  "questionnaire": [
    {
