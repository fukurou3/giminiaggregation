...
export const CategorySidebar = React.memo<CategorySidebarProps>(function CategorySidebar({
  ...
  // Helper to identify mobile phases
  const isMobilePhase = layoutPhase === 'phase4' || layoutPhase === 'phase5';
  return (
    <div
      ref={sidebarRef}
      className={`
      ...
      `}
      style={{
        display: isMobilePhase ? undefined : 'block',
        position: isMobilePhase ? 'fixed' : (isSidebarFixed ? 'fixed' : 'static'),
        top: isMobilePhase ? '0' : (isSidebarFixed ? '80px' : undefined),
        height: isMobilePhase ? '100vh' : undefined,
        zIndex: isMobilePhase ? 40 : (isSidebarFixed ? 10 : undefined),
      }}
    >
      <div 
      ...
      >
        ...
        <div 
          className={isMobilePhase ? "pt-23 px-3 pb-3" : "px-3 py-8"}
          style={{width: '190px', minWidth: '190px', maxWidth: '190px'}}
        >
          {/* Desktop Header - Show in all phases except mobile */}
          <h2 className={`${isMobilePhase ? 'hidden' : 'block'} text-base font-semibold text-foreground mb-3 flex items-center`}>
